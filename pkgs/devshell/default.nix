{pkgs ? import <nixpkgs> {}}: let
  ...
  languagePackages = {
    python = with pkgs; lib.optionals features.python ([
      # Core Python
      python3
      python3Packages.pip
    ...
    rust = with pkgs; lib.optionals features.rust ([
      # Core Rust toolchain
      rustc
      cargo
      rustfmt
    ...
    go = with pkgs; lib.optionals features.go ([
      # Core Go
      go
    ...
    node = with pkgs; lib.optionals features.node [
      # Core Node.js
      nodejs
      nodePackages.npm
      nodePackages.yarn
    ...
    lua = with pkgs; lib.optionals (features.lua or false) [
      lua
      luarocks
      lua-language-server
      stylua
    ...
    nix = with pkgs; lib.optionals (features.nix or true) [
